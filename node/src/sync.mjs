...
export class SyncHandler {
    ...
    /** @param {Node} node */
    constructor(node) {
        node.p2pNetwork.p2p.handle(PROTOCOLS.SYNC, this.#handleIncomingStream);
        this.node = node;
    ...
    async #sendSyncRequest(peerIdStr, msg, maxSuccessiveFailures = 5) {
        ...
        while (true) { // Wait peer to be dialable at first...
            ...
            try { // try to get the remaining data
                ...
                stream = await this.p2pNet.p2p.dialProtocol(peer.id, PROTOCOLS.SYNC, { signal: AbortSignal.timeout(3_000) });
