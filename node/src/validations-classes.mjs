...
export class TxValidation {
    ...
    static isConformTransaction(involvedUTXOs, transaction, specialTx, nodeVersion) {
        ...
        try {
            for (const witness of transaction.witnesses)
				TxValidation.#decomposeWitnessOrThrow(witness);
    ...
    static controlAllWitnessesSignatures(memPool, transaction) {
        ...
        for (let i = 0; i < transaction.witnesses.length; i++) {
            ...
            AsymetricFunctions.verifySignature(signature, transaction.id, pubKeyHex); // will throw an error if the signature is invalid
    ...
    static async fullTransactionValidation(involvedUTXOs, memPool, transaction, specialTx, nodeVersion, useDevArgon2 = false) {
        ...
        if (memPoolTx)
            for (let i = 0; i < memPoolTx.inputs.length; i++)
                if (memPoolTx.inputs[i] !== transaction.inputs[i]) {
					result.success = true; 
					return result;
				}
        
        const impliedKnownPubkeysAddresses = TxValidation.controlAllWitnessesSignatures(memPool, transaction);
...
export class BlockValidation {
    ...
    static async fullBlockTxsValidation(blockData, utxoCache, memPool, workers) {
        ...
        if (nbOfWorkers === 0 || blockData.Txs.length <= minTxsToUseWorkers) {
            for (let i = 0; i < blockData.Txs.length; i++) {
                ...
                const { fee, success, discoveredPubKeysAddresses } = await TxValidation.fullTransactionValidation(involvedUTXOs, memPool, tx, specialTx, utxoCache.nodeVersion);
        ...
        for (let i = 0; i < blockData.Txs.length; i++) {
            ...
            const { fee, success, impliedKnownPubkeysAddresses } = await TxValidation.partialTransactionValidation(
				involvedUTXOs, memPool, tx, specialTx, utxoCache.nodeVersion
			);
            if (!success) throw new Error(`Invalid transaction: ${tx.id}`);

			for (const pubKeyHex in impliedKnownPubkeysAddresses)
				allImpliedKnownPubkeysAddresses[pubKeyHex] = impliedKnownPubkeysAddresses[pubKeyHex];
        }
        ...
        for (let i = 0; i < 2; i++) {
            ...
            const { fee, success, discoveredPubKeysAddresses } = await TxValidation.fullTransactionValidation(
				involvedUTXOs, memPool, tx, specialTx, utxoCache.nodeVersion
			);
            if (!success) throw new Error(`Invalid transaction: ${tx.id}`);
        ...
        for (const worker of workers) {
            ...
            workersPromises[worker.id] = worker.addressOwnershipConfirmation(
                involvedUTXOs, txs, allImpliedKnownPubkeysAddresses, useDevArgon2
            );
        }
