...
await bootstrapWallet.deriveAccounts(nbReceipients, 'W', bootstrapStorage);
...
while(true) {
	...
	for (let i = 0; i < nbReceipients; i++) {
		const receipient = bootstrapWallet.accounts.W[i].address;
		transfers.push(new Transfer(receipient, 1_000));
	}
	
	const a = bootstrapWallet.accounts.C[0];
	a.ledgerUtxos = bootstrapNode.blockchain.ledgersStorage.getAddressLedger(a.address).ledgerUtxos;
	...
	try { await bootstrapNode.memPool.pushTransaction(bootstrapNode, signedTx); }
	catch (error) { console.error('Failed to push transaction to mempool:', error.message); }

	// TEST: SEND LOT OF SINGLE OUTPUT TRANSACTIONS
	let txs = [];
	for (let i = 0; i < nbOfSenders; i++) {
		await new Promise((resolve) => setTimeout(resolve, 10)); // avoid freeze
		
		const account = bootstrapWallet.accounts.W[i];
		account.ledgerUtxos = bootstrapNode.blockchain.ledgersStorage.getAddressLedger(account.address).ledgerUtxos;
		const receipient = bootstrapWallet.accounts.C[0].address; // send back to main account
		const signedTx2 = Transaction_Builder.createAndSignTransaction(account, 100, receipient, 1)?.signedTx;
		if (signedTx2) txs.push(signedTx2);
	}

	let nbSent = 0;
	for (const tx of txs) { // FLOOD!
		try { await bootstrapNode.memPool.pushTransaction(bootstrapNode, tx); nbSent++; }
		catch (error) { console.error('Failed to push transaction to mempool:', error.message); }
	}
