...
export const SIZES = {
	...
	// BLOCK VALUES
	/** Block candidate header, represented as 60 bytes, containing: nbOfTxs(2) + index(4) + supply(6) + coinBase(4) + difficulty(4) + legitimacy(2) + prevHash(64) + posTimestamp(6) + powReward(6) */
	blockCandidateHeader: { bytes: 2 + 4 + 6 + 4 + 4 + 2 + 64 + 6 + 6, str: null },
	/** Block finalized header, represented as 96 bytes, containing: nbOfTxs(2) + index(4) + supply(6) + coinBase(4) + difficulty(4) + legitimacy(2) + prevHash(64) + posTimestamp(6) + timestamp(6) + hash(64) + nonce(4) */
	blockFinalizedHeader: { bytes: 2 + 4 + 6 + 4 + 4 + 2 + 64 + 6 + 6 + 64 + 4, str: null },
	/** Hash, represented as 64 bytes or 128 hex characters */
	hash: { bytes: 64, str: 128 },
